...
@author: jordanh, Axel Roest
'''
## startup : python xigudp.py

## Global String Constants
NAME = "XBee Internet Gateway (XIG) + UDP+OSC"
SHORTNAME = "xigudp"
VERSION = "1.1.3a"
...
import sys, time, os, zipimport

# http://www.digi.com/wiki/developer/index.php/Digi_Python_Programmer%27s_Guide#Loading_Python_Programs_onto_a_Digi_Device
APP_ARCHIVE = "WEB/python/_xig.zip"
print sys.path 
if APP_ARCHIVE in zipimport._zip_directory_cache:
    del zipimport._zip_directory_cache[APP_ARCHIVE]
sys.path.insert(0, APP_ARCHIVE)
# sys.path.append(APP_ARCHIVE)
print sys.path 
...
from oscsession import OSCSession
print 'done.'
...
COMMANDS:
 ...
 experimental:
 udp://<host:port>/path> sends the path to the host using udp and doesn't wait for a response
 osc:///message/path?parameter1&param2
 osc://<host:port>/message/path?parameter1&param2
...
class XigUdp(object):
    def __init__(self):
        ...
        """
                self.__quit_sock = socket(AF_INET, SOCK_STREAM)
                self.__quit_sock.bind(("", 32000))
                self.__quit_sock.listen(1)

            # handy to quit the router by sending a packet to port 32000
            # only it doesn't work (yet)
            def checkQuitPacket(self):
                rl, wl, el = select.select([quit_sock], [], [], 0)
                if quit_sock in rl:
                    self.quit()
...
class XigIOKernel(object):
    ...
    def __init__(self, xig_core):
        ...
        self.__session_classes = (
          ...
          UDPSession,           # for testing, AXEL
          OSCSession,           # for testing, AXEL
    ...
    def ioLoop(self, timeout=0):
        ...
        if self.__xbee_sd in rl:
            ...
            # print "RECV: %d bytes [%s] from %s" % (len(buf), repr(buf), repr(addr[0:4]))
...
# from Digi support, to prevent the dreaded "zipimport.ZipImportError: bad local file header in WEB/python/_xig.zip" error
...
def main():
    # take off every Xig!
    ## see if this solves the dreaded TypeError: 'module' object is not callable
    xig = XigUdp()
    ret = xig.go()
    sys.exit(ret)
